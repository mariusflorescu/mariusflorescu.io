...
To give you a little bit of context, what I wanted to achieve is a link preview component, and if you are not familiar with what or how a link preview should behave, you can check out [this article](https://uiw.tf/link-preview).

Besides a _pleasing UI_, I wanted to have screenshots that were resized to according dimensions (not too large, since they are going to be shown as small images), saved as `webp` (also to keep them as tiny as possible) and also regularly updated.

The need of keeping them updated was due to the fact that some pages from certain application may often change (such as a twitter profile) and I wanted the screenshots to be as close to present as possible.

#### Note

For those who are not familiar with that image format:

> WebP is a modern image format that provides superior lossless and lossy compression for images on the web. Using WebP, webmasters and web developers can create smaller, richer images that make the web faster.

## Generating screenshots with Github Actions

The first thing that came to my mind while I was thinking about a mechanism that is going to automatically generate the screenshots was **Github Actions**.
...
After the screenshots of both themes is done, we are going to resize the images and save them with the `webp` extension. To resize the images I'm using [sharp](https://www.npmjs.com/package/sharp), feel free to check it out.

#### Note

I'm using `browserless` due to its strong configuration out of the box, which is _tricks_ many applications against their bot protection. As an example, trying to screenshots your _Twitter_ account with either Puppeteer or Playwright with no prior configuration will result in screenshots that are going to look like this:

INSERT IMG

### Room for improvements

This action seems to run perfectly, but we must ask ourselves what happens if the Headless Browser cannot connect to the website or something else goes wrong ?In the current state, the script would simply fail and not update the repository.
