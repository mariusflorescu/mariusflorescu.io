...
Recently, link previews have increased in popularity, and I'm seeing them more frequently, particularly on technology-related websites. I believe that having a small box with an image of the destination that pops out whenever you hover over the external link is an _excellent_ concept, and one that I would like to see more frequently, as it is a very subtle touch.

There are a variety of approaches for generating a link preview component; let's have a look at a few of them and see what advantages and disadvantages they possess.

## Using Microlink's screenshot API

This is without a doubt the most _popular_ approach that I've come across so far, owing to the fact that it's quite simple to use and that the images are updated on a frequent basis.
...
Below is an implementation of the `LinkPreview` component in Next.js. Additionally, I am using the `Image` component from Next.js, the `HoverCard` primitive from [Radix-UI](https://www.radix-ui.com/docs/primitives/components/hover-card) and the `encode` from qss package, but feel free to play with it.
...
Don't forget to add the Microlink API in your domains array from the `next.config.js` !
...
A good yet _simple_ method in which you take screenshots from all websites that you wish to preview and throw them into the `public/` folder of the app.
