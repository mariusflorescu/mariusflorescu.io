...
publishedAt: "Jan 01"
description: "Link previews are a fun and nice to have feature that increase the UX by a lot, but let's see some options of creating them."
status: "private"
---
...
The reason why I'm not using the `Image` component from Next.js is because until it won't be visible the optimizing won't be done, so for the first user that comes and tries the link preview it will _flicker_ the image in order to cache it.
